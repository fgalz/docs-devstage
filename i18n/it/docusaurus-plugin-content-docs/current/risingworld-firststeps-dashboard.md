...
Il **Registro Eventi** ti offre una panoramica completa di tutte le attività legate all’amministrazione del server. Qui puoi vedere quando un server è stato avviato, fermato o crashato, così come quando sono stati eseguiti aggiornamenti, backup o nuove installazioni.
...
Il tuo server è stato offline temporaneamente e non sai perché? Il registro eventi e i file di log non hanno dato informazioni? Potrebbe essere che la causa non sia il tuo server, ma il sistema host su cui gira.
...
Sotto **Versioni** puoi vedere e gestire la versione attuale del tuo server di gioco. Qui puoi aggiornare all’ultima versione o attivare gli aggiornamenti automatici per mantenere il server sempre aggiornato.
...
Vuoi programmare riavvii del server a orari specifici? Il Pianificatore Riavvii ti permette di impostare riavvii automatici programmati. Puoi scegliere orari specifici o intervalli regolari.
...
Oltre ai server di gioco, offriamo anche 10 database inclusi. Questi sono necessari, per esempio, quando i dati devono essere salvati in un database. Sotto **Database** puoi crearne uno e vedere le credenziali di accesso. Puoi anche attivare i backup e importare backup esistenti.
...
Nelle impostazioni puoi scegliere se creare backup automatici, con frequenza giornaliera o settimanale. Puoi anche decidere se eliminare i backup più vecchi quando lo spazio di archiviazione è limitato.
